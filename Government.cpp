...
void Government::collectBuildingTax()
{
     int i=1;
      ...
      City* myCitizen=new City(buildings,citizens);

      CityIterator* myIterator=myCitizen->createBuildingIterator();
       while(myIterator->hasNext()){
         if(myIterator->buildingCurrentItem()->gethasbool() && myIterator->buildingCurrentItem()->gethasSolar()){
         double tax= myIterator->buildingCurrentItem()->totalAmount() * buildingTaxRate;
      //CTotalTax+=tax;
       BTotalTax+=tax;
            std::cout <<"( "<< i++<< ") Collected R" << tax << " from building." << std::endl;
...
void Government::collectCitizenTax() {
      ...
      CityIterator* myIterator=myCitizen->createCitizenIterator();
      while(myIterator->hasNext()){
        if(myIterator->citizenCurrentItem()->getIncome() && myIterator->citizenCurrentItem()->getEmployment()){
            double tax= myIterator->citizenCurrentItem()->getIncome() * citizenTaxRate;
      CTotalTax+=tax;
            std::cout <<"( "<< i++<< ") Collected  R: " << tax << " from citizen." << std::endl;
 ...
 collectCitizenTax();
    collectBuildingTax();
    double totalTax=CTotalTax+BTotalTax;
    std::cout << "City tax collection complete. Total :R " <<totalTax<< std::endl;
 }

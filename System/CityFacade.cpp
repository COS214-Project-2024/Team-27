...
void CityFacade::startUp()
{
    ...
    int integer = (apartmentBuildings.size() + houses.size() + landmarks.size() + commercialBuildings.size()) ;

    growthManager = new CityGrowthManager(citizens.size(),integer , 1);
...
void CityFacade::AssignUtilitiestoBuildings()
{
    ...
    while (loop)
    {
        ...
        if (R == 1 || R == 2)
        {
            if (R == 1)
            {
                if(without1.empty() && without2.empty() && without3.empty() && without4.empty()){
                    std::cout<<"the is no buildings to add utilities to"<<std::endl;
                }
...
void CityFacade::saveState(){
    government = new Government(citizens, apartmentBuildings, houses, landmarks, commercialBuildings);

    state = government->saveState(citizens, apartmentBuildings, houses, landmarks, commercialBuildings);

    cout<<"State saved"<<endl;
}

void CityFacade::loadCityStates(){
    if(state){
        apartmentBuildings = state->getAPBuildings();
        houses = state->getHBuildings();
        landmarks = state->getLBuildings();
        commercialBuildings = state->getComBuildings();
        citizens = state->getCitizens();

        cout<<"City state loaded"<<endl;
    } else{
        cout<<"No state to load"<<endl;
    }
}